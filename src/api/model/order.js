...
module.exports = class extends think.Model {
  ...
  async consumeTran(data){
    return await this.transaction(async () => {
      await this.where({id:data.orderId}).update({order_status:201})
      const voucher = this.model('voucher').db(this.db());
      await voucher.addMany(data.voucher);
      const user = this.model('user').db(this.db());
      await user.where({id:think.userId}).decrement('point', data.orderInfo.actual_price*100)
      return "success"
    })
  }
  /**
   * 获取订单可操作的选项
   * @param orderId
   * @returns {Promise.<{cancel: boolean, delete: boolean, pay: boolean, comment: boolean, consume: boolean, confirm: boolean, return: boolean}>}
   */
  ...
  async getOrderStatusText(orderId,fromDB=true) {
    let order_status = orderId
    if (fromDB) {
      const orderInfo = await this.where({id: orderId}).find();
